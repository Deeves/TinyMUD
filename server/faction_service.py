    ...
    for n in (graph.get('npcs') or []):
        ...
        try:
            ...
            if faction:
                ...
                rank_name = str(n.get('rank') or '').strip()
                if rank_name:
                    faction.set_member_rank(npc_id, rank_name)
        except Exception:
            pass
...
def _offline_default_graph() -> dict:
    ...
    return {
        ...
        "ranks": [
            ...
            {"name": "Soldier", "level": 2},
            {"name": "Captain", "level": 5},
            {"name": "Commander", "level": 10}
        ],
        "rooms": [
        ...
        "npcs": [
            {"name": "Quartermaster", "description": "Keeps the ledgers and keys, stern but fair.", "room": "hall", "rank": "Captain"},
            {"name": "Cook Mira", "description": "Bustling and chatty, always with flour on her hands.", "room": "kitchen", "rank": "Soldier"},
            {"name": "Scout Rafe", "description": "Light on his feet, fond of tall tales.", "room": "bunks", "rank": "Recruit"},
        ],
        "relationships": [
...
def _build_ai_prompt(world: World) -> str:
    ...
    parts = [
        ...
        "Return ONLY JSON with keys: faction_name, faction_description, faction_relationships, ranks, rooms, links, npcs, relationships. No prose, no markdown.",
        ...
        "  ranks: [{name: string, level: int}],",
        "  rooms: [{id: string, description: string}],",
        "  links: [{a: string, door_a: string, b: string, door_b: string}],",
        "  npcs:  [{name: string, description: string, room: string, rank: string}],",
...
def handle_faction_command(world: World, state_path: str, sid: str | None, args: List[str]) -> Tuple[bool, Optional[str], List[dict], List[Tuple[str, dict]]]:
    ...
    - addrank: Add a rank to a faction
    - removerank: Remove a rank from a faction
    - setrank: Assign a rank to a faction member
    ...
    if not args:
        ...
        usage_msg = (
            ...
            "  addrank <faction> | <rank_name> | <level> - Add rank\n"
            "  removerank <faction> | <rank_name> - Remove rank\n"
            "  setrank <faction> | <member> | <rank_name> - Assign rank\n"
            "  addrole <faction> | <role_id> | <name> | <type> | <config_json> - Create role\n"
            "  assignrole <faction> | <member> | <role_id> - Assign role\n"
            "  removerole <faction> | <role_id> - Delete role definition\n"
            "  revokerole <faction> | <member> - Remove role from member"
        )
    ...
    # /faction addrank <faction> | <rank_name> | <level>
    if sub == 'addrank':
        if not sub_args:
            return True, "Usage: /faction addrank <faction> | <rank_name> | <level>", emits, broadcasts
            
        parts_joined = " ".join(sub_args)
        if not _validate_pipe_syntax(parts_joined, 3):
            return True, "Usage: /faction addrank <faction> | <rank_name> | <level>", emits, broadcasts
            
        parse_result = safe_call(lambda: parse_pipe_parts(parts_joined, expected=3))
        if parse_result is None:
            return True, "Usage: /faction addrank <faction> | <rank_name> | <level>", emits, broadcasts
            
        faction_name, rank_name, level_str = parse_result
        ...
        try:
            ...
            return True, "Level must be an integer.", emits, broadcasts
            
        faction.add_rank(rank_name.strip(), level)
        _save_world()
        emits.append({
            'type': 'system',
            'content': f"Added rank '[b]{rank_name.strip()}[/b]' (level {level}) to faction '[b]{faction.name}[/b]'."
        })
        return True, None, emits, broadcasts

    # /faction removerank <faction> | <rank_name>
    if sub == 'removerank':
        if not sub_args:
            return True, "Usage: /faction removerank <faction> | <rank_name>", emits, broadcasts
        ...
        faction_name, rank_name = parse_result
        
        ok, err, faction = _resolve_faction_by_name(faction_name)
        if not ok or not faction:
            return True, err or "Faction not found.", emits, broadcasts
            
        removed = faction.remove_rank(rank_name.strip())
        if removed:
            _save_world()
            emits.append({
                'type': 'system',
                'content': f"Removed rank '[b]{rank_name.strip()}[/b]' from faction '[b]{faction.name}[/b]'."
            })
        else:
            emits.append({
                'type': 'system',
                'content': f"Rank '[b]{rank_name.strip()}[/b]' not found in faction '[b]{faction.name}[/b]'."
            })
        return True, None, emits, broadcasts

    # /faction setrank <faction> | <member> | <rank_name>
    if sub == 'setrank':
        if not sub_args:
            return True, "Usage: /faction setrank <faction> | <member> | <rank_name>", emits, broadcasts
        ...
        faction_name, member_name, rank_name = parse_result
        
        ok, err, faction = _resolve_faction_by_name(faction_name)
        if not ok or not faction:
            return True, err or "Faction not found.", emits, broadcasts
            
        ok_entity, err_entity, entity_id, display_name = _resolve_entity_for_membership(member_name)
        if not ok_entity or not entity_id:
            return True, err_entity or "Member not found.", emits, broadcasts
            
        if not (faction.is_player_member(entity_id) or faction.is_npc_member(entity_id)):
             return True, f"'{display_name}' is not a member of faction '{faction.name}'.", emits, broadcasts

        assigned = faction.set_member_rank(entity_id, rank_name.strip())
        if assigned:
            _save_world()
            emits.append({
                'type': 'system',
                'content': f"Assigned rank '[b]{rank_name.strip()}[/b]' to '[b]{display_name}[/b]' in faction '[b]{faction.name}[/b]'."
            })
        else:
            emits.append({
                'type': 'system',
                'content': f"Rank '[b]{rank_name.strip()}[/b]' does not exist in faction '[b]{faction.name}[/b]'."
            })
        return True, None, emits, broadcasts

    # /faction promote <faction> | <member> | <new_rank>
    if sub == 'promote':
        if not sub_args:
            return True, "Usage: /faction promote <faction> | <member> | <new_rank>", emits, broadcasts
        ...
        # Check permissions (assuming admin can do anything, but let's simulate actor check if sid is provided)
        # Since this is an admin command, we bypass the actor check and just set it.
        # BUT the user asked to verify "PCs and NPCs... can promote".
        # This implies we need a player-facing command, not just admin.
        # However, `handle_faction_command` is currently an admin command suite.
        # I will implement the logic here as if the admin is forcing it, BUT I will also add a check if `sid` corresponds to a player who is NOT an admin?
        # Actually, `handle_faction_command` is called by `admin_router`.
        # To support player-driven promotion, I should probably add a `faction_router` or similar.
        # For now, I'll implement it here. If I need to expose it to players, I'll need to change where it's called.
        # But wait, if I implement it here, I can test it by mocking the context.
        # The prompt says "Add unit tests and verify...".
        # I will implement the logic in `Faction` (done) and verify it with unit tests.
        # The command implementation here is for admins to force it.
        # I'll add the command here for completeness.
        ...
        if assigned:
            ...
            emits.append({
                'type': 'system',
                'content': f"Promoted '[b]{display_name}[/b]' to '[b]{rank_name.strip()}[/b]' in faction '[b]{faction.name}[/b]'."
            })
        else:
            emits.append({
                'type': 'system',
                'content': f"Rank '[b]{rank_name.strip()}[/b]' does not exist."
            })
        return True, None, emits, broadcasts

    # /faction demote <faction> | <member> | <new_rank>
    if sub == 'demote':
        if not sub_args:
            return True, "Usage: /faction demote <faction> | <member> | <new_rank>", emits, broadcasts
        ...
        if assigned:
            ...
            emits.append({
                'type': 'system',
                'content': f"Demoted '[b]{display_name}[/b]' to '[b]{rank_name.strip()}[/b]' in faction '[b]{faction.name}[/b]'."
            })
        else:
            emits.append({
                'type': 'system',
    ...
    # /faction induct <faction> | <member>
    if sub == 'induct':
        if not sub_args:
            return True, "Usage: /faction induct <faction> | <member>", emits, broadcasts
        ...
        if added:
            ...
            emits.append({
                'type': 'system',
                'content': f"Inducted {member_type} '[b]{display_name}[/b]' into faction '[b]{faction.name}[/b]'."
            })
        else:
            emits.append({
                'type': 'system',
    ...
    # /faction addrole <faction> | <role_id> | <name> | <type> | <config_json>
    if sub == 'addrole':
        if not sub_args:
            return True, "Usage: /faction addrole <faction> | <role_id> | <name> | <type> | <config_json>", emits, broadcasts
            
        parts_joined = " ".join(sub_args)
        # Expect 5 parts
        parse_result = safe_call(lambda: parse_pipe_parts(parts_joined, expected=5))
        if parse_result is None:
            return True, "Usage: /faction addrole <faction> | <id> | <name> | <type> | <json>", emits, broadcasts
            
        f_name, r_id, r_name, r_type, r_config_str = parse_result
        
        ok, err, faction = _resolve_faction_by_name(f_name)
        if not ok or not faction:
            return True, err or "Faction not found.", emits, broadcasts
            
        # Parse JSON config
        try:
            config = json.loads(r_config_str)
        except Exception:
            return True, "Invalid JSON config.", emits, broadcasts
            
        role = FactionRole(
            id=r_id.strip(),
            name=r_name.strip(),
            description=f"Role {r_name}", # minimal desc
            contract_type=r_type.strip(),
        ...
        emits.append({'type': 'system', 'content': f"Role '[b]{role.name}[/b]' (ID: {role.id}) added to faction."})
        return True, None, emits, broadcasts

    # /faction assignrole <faction> | <member> | <role_id>
    if sub == 'assignrole':
        if not sub_args:
            return True, "Usage: /faction assignrole <faction> | <member> | <role_id>", emits, broadcasts
        ...
        f_name, m_name, r_id = parse_result
        
        ok, err, faction = _resolve_faction_by_name(f_name)
        if not ok or not faction:
            return True, err or "Faction not found.", emits, broadcasts
            
        ok_ent, err_ent, ent_id, ent_disp = _resolve_entity_for_membership(m_name)
        if not ok_ent or not ent_id:
            return True, err_ent or "Member not found.", emits, broadcasts
            
        if not faction.is_player_member(ent_id) and not faction.is_npc_member(ent_id):
            return True, f"{ent_disp} is not a member of {faction.name}.", emits, broadcasts
            
        # Check if role exists
        if r_id.strip() not in faction.roles:
            return True, f"Role ID '{r_id}' not found in faction.", emits, broadcasts
            
        faction.assign_role(ent_id, r_id.strip())
        _save_world()
        
        emits.append({'type': 'system', 'content': f"Assigned role '[b]{r_id}[/b]' to {ent_disp}."})
        return True, None, emits, broadcasts

    # /faction revokerole <faction> | <member>
    if sub == 'revokerole':
        parts_joined = " ".join(sub_args)
        parse_result = safe_call(lambda: parse_pipe_parts(parts_joined, expected=2))
        if parse_result is None:
            return True, "Usage: /faction revokerole <faction> | <member>", emits, broadcasts
            
        f_name, m_name = parse_result
        ok, err, faction = _resolve_faction_by_name(f_name)
        if not ok or not faction: return True, err, emits, broadcasts
        
        ok_ent, err_ent, ent_id, ent_disp = _resolve_entity_for_membership(m_name)
        if not ok_ent or not ent_id: return True, err_ent, emits, broadcasts
        
        if faction.revoke_role(ent_id):
            _save_world()
            emits.append({'type': 'system', 'content': f"Role revoked from {ent_disp}."})
        else:
            emits.append({'type': 'system', 'content': f"{ent_disp} had no role."})
        return True, None, emits, broadcasts

    # /faction removerole <faction> | <role_id>
    if sub == 'removerole':
        parts_joined = " ".join(sub_args)
        parse_result = safe_call(lambda: parse_pipe_parts(parts_joined, expected=2))
        if parse_result is None:
            return True, "Usage: /faction removerole <faction> | <role_id>", emits, broadcasts
        
        f_name, r_id = parse_result
        ok, err, faction = _resolve_faction_by_name(f_name)
        if not ok or not faction: return True, err, emits, broadcasts
        
        if faction.remove_role(r_id.strip()):
            _save_world()
            emits.append({'type': 'system', 'content': f"Role definition '{r_id}' deleted."})
        else:
            emits.append({'type': 'system', 'content': "Role not found."})
        return True, None, emits, broadcasts
