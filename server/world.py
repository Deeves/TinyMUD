...
class CharacterSheet:
    ...
    # --- Self-Actualization Layer ---
    ambition: Optional[Ambition] = None
    ...
    def to_dict(self) -> dict:
        d = {
            ...
            # Self-Actualization
            "ambition": self.ambition.to_dict() if self.ambition else None,
    ...
    def from_dict(data: dict) -> "CharacterSheet":
        ...
        return CharacterSheet(
            ...
            # Self-Actualization
            ambition=Ambition.from_dict(data.get("ambition")) if data.get("ambition") else None,
            lifetime_stats=dict(data.get("lifetime_stats", {})),
        )
...
class Faction:
    ...
    def __init__(self, 
                 ...
                 rival_faction_ids: Optional[List[str]] = None,
                 ranks: Optional[Dict[str, int]] = None,
                 member_ranks: Optional[Dict[str, str]] = None,
                 roles: Optional[Dict[str, FactionRole]] = None,
                 member_roles: Optional[Dict[str, str]] = None,
                 leadership_threshold: int = 5) -> None:
        ...
        Args:
            ...
            ranks: Dictionary of rank names to level (int)
            member_ranks: Dictionary of member IDs to rank names
            leadership_threshold: Minimum rank level required to induct new members (default 5)
        """
        ...
        # Ranks system
        self.ranks: Dict[str, int] = ranks or {}
        self.member_ranks: Dict[str, str] = member_ranks or {}
        
        # New: Roles system (Functional Jobs)
        # roles: role_id -> FactionRole definition
        self.roles: Dict[str, FactionRole] = roles or {}
        # member_roles: member_id -> role_id
        self.member_roles: Dict[str, str] = member_roles or {}
        
        self.leadership_threshold: int = leadership_threshold
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'leadership_threshold': self.leadership_threshold,
    ...
    def from_dict(cls, data: dict) -> "Faction":
        ...
        roles = {}
        if roles_data:
            for r_id, r_data in roles_data.items():
                roles[r_id] = FactionRole.from_dict(r_data)
        ...
        faction = cls(
            ...
            leadership_threshold=data.get('leadership_threshold', 5)
        )
    ...
    # --- Role Management ---
    def add_role(self, role: FactionRole) -> None:
        """Add or update a role definition."""
        self.roles[role.id] = role

    def remove_role(self, role_id: str) -> bool:
        """Remove a role definition and revoke it from all holders."""
        if role_id in self.roles:
            del self.roles[role_id]
            # Revoke from all members
            for member_id in list(self.member_roles.keys()):
                if self.member_roles[member_id] == role_id:
                    del self.member_roles[member_id]
            return True
        return False

    def assign_role(self, member_id: str, role_id: str) -> bool:
        """Assign a role to a member."""
        if role_id not in self.roles:
            return False
        if member_id in self.member_player_ids or member_id in self.member_npc_ids:
            self.member_roles[member_id] = role_id
            return True
        return False
        
    def revoke_role(self, member_id: str) -> bool:
        """Remove role from a member."""
        if member_id in self.member_roles:
            del self.member_roles[member_id]
            return True
        return False
        
    def get_member_role(self, member_id: str) -> Optional[FactionRole]:
        """Get the Role object assigned to a member."""
        role_id = self.member_roles.get(member_id)
        if role_id:
            return self.roles.get(role_id)
        return None

    def add_rank(self, rank_name: str, level: int) -> bool:
        """Add or update a rank.
        
        Args:
            rank_name: Name of the rank
            level: Integer level (higher is more senior)
            
        Returns:
            True
        """
        self.ranks[rank_name] = level
        return True

    def remove_rank(self, rank_name: str) -> bool:
        """Remove a rank.
        ...
        if rank_name in self.ranks:
            del self.ranks[rank_name]
            # Remove this rank from any members who have it
            for member_id in list(self.member_ranks.keys()):
                if self.member_ranks[member_id] == rank_name:
                    del self.member_ranks[member_id]
            return True
        return False

    def set_member_rank(self, member_id: str, rank_name: str) -> bool:
        """Assign a rank to a member.
        
        Args:
            member_id: The member's ID
            rank_name: The rank to assign
            
        Returns:
            True if assigned, False if rank doesn't exist or member not in faction
        """
        if rank_name not in self.ranks:
            return False
        if member_id in self.member_player_ids or member_id in self.member_npc_ids:
            self.member_ranks[member_id] = rank_name
            return True
        return False

    def get_member_rank(self, member_id: str) -> Optional[str]:
        """Get the rank name of a member.
        ...
        Returns:
            Rank name or None
        """
        return self.member_ranks.get(member_id)

    def get_member_level(self, member_id: str) -> int:
        """Get the numeric rank level of a member.
        
        Args:
            member_id: The member's ID
            
        Returns:
            Rank level (int), or 0 if no rank/not found
        """
        rank_name = self.member_ranks.get(member_id)
        if rank_name and rank_name in self.ranks:
            return self.ranks[rank_name]
        return 0

    def can_promote(self, actor_id: str, target_id: str, new_rank: str) -> bool:
        """Check if actor can promote target to new_rank.
        
        Rules:
        - Actor must be higher status than target.
        - Actor must be higher status than the new rank (cannot promote to equal/above self).
        - New rank must be higher than target's current rank.
        """
        if new_rank not in self.ranks:
            return False
        actor_level = self.get_member_level(actor_id)
        target_level = self.get_member_level(target_id)
        new_level = self.ranks[new_rank]
        
        return (actor_level > target_level) and (actor_level > new_level) and (new_level > target_level)

    def can_demote(self, actor_id: str, target_id: str, new_rank: str) -> bool:
        """Check if actor can demote target to new_rank.
        
        Rules:
        - Actor must be equal or higher status than target.
        - New rank must be lower than target's current rank.
        """
        if new_rank not in self.ranks:
            return False
        actor_level = self.get_member_level(actor_id)
        ...
        return (actor_level >= target_level) and (new_level < target_level)

    def can_induct(self, actor_id: str) -> bool:
        """Check if actor can induct new members.
        
        Rules:
        - Actor is the designated leader OR
        - Actor's rank level >= leadership_threshold
        """
        if self.leader_player_id and actor_id == self.leader_player_id:
            return True
        return self.get_member_level(actor_id) >= self.leadership_threshold
