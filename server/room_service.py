...
def handle_room_command(world: "World", state_path: str, args: list[str], sid: str | None = None) -> Tuple[bool, str | None, List[dict], List[Tuple[str, dict]]]:
    ...
    if sub == 'delete':
        if not sub_args:
            return True, 'Usage: /room delete <id>', emits, broadcasts
        room_id = _strip_quotes(sub_args[0])
        if room_id not in world.rooms:
            return True, f"Room '{room_id}' not found.", emits, broadcasts
        
        # Safety: don't delete if players are here
        room = world.rooms[room_id]
        if room.players:
            return True, f"Cannot delete room '{room_id}': players are present.", emits, broadcasts
            
        del world.rooms[room_id]
        _save_silent(world, state_path)
        emits.append({'type': 'system', 'content': f"Room '{room_id}' deleted."})
        return True, None, emits, broadcasts
